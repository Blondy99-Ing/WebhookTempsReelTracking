...
async function syncCursorsWithDb({
  db,
  state,
  logger,
  startFromNow = true,
  ...
  if (!state.lastLocationId || state.lastLocationId === 0) {
    if (startFromNow) {
      ...
      changed = true;
      logger.warn(
        "[CURSOR] lastLocationId was 0 → start from NOW"
      );
    }
    ...
    changed = true;
    logger.warn(
      "[CURSOR] lastLocationId > DB max → reset"
    );
  }
  ...
  if (!state.lastAlertId || state.lastAlertId === 0) {
    if (startFromNow) {
      ...
      changed = true;
      logger.warn(
        "[CURSOR] lastAlertId was 0 → start from NOW"
      );
    }
    ...
    changed = true;
    logger.warn(
      "[CURSOR] lastAlertId > DB max → reset"
    );
  }
